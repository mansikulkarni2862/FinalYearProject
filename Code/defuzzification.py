...
def defuzzification(output_dict):
    ...
    if output_dict['outputsick_healthy'] > 0:  # if the output of the fuzzy system is 1, the output is healthy
        healthy = health_defuzzy(output_dict['outputsick_healthy'])
        numerator += healthy[0]
        denominator += healthy[1]
    if output_dict['outputsick_sick1'] > 0:
        sick_1 = sick1_defuzzy(output_dict['outputsick_sick1'])
        numerator += sick_1[0]
        denominator += sick_1[1]
    # ... (similar logic for outputsick_sick2, etc.)
    ...
    # Mapping center of gravity to health risk categories
    if centerOfGravity < 1.78:
        total_res = 'healthy'
    elif 1.78 <= centerOfGravity < 3.25:
        total_res = 'mid risk'
    else:
        total_res = 'high risk'

    # Adding center of gravity (optional)
    # total_res += ' : ' + str(round(centerOfGravity, 3))  # uncomment to include
